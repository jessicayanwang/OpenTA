...
export default function ProfessorConsole() {
  ...
  useEffect(() => {
    ...
    // Load data - demo data is seeded once at backend startup
    loadData()
  }, [activeTab])
  ...
  const handlePublishAnswer = async () => {
    ...
    try {
      // Create the canonical answer
      const createRes = await fetch('http://localhost:8000/api/professor/canonical-answer', {
        method: 'POST',
      ...
      const answerData = await createRes.json()
      
      // Automatically publish the answer
      await fetch(`http://localhost:8000/api/professor/canonical-answer/${answerData.answer_id}/publish`, {
        method: 'POST'
      })
